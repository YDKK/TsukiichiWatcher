...
WORKDIR /src
COPY ["TsukiichiWatcher.csproj", "TsukiichiWatcher/"]
RUN dotnet restore "TsukiichiWatcher/TsukiichiWatcher.csproj"

WORKDIR "/src/TsukiichiWatcher"
COPY . .
RUN dotnet build "TsukiichiWatcher.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "TsukiichiWatcher.csproj" \
...
Execute unit tests
```
cd "TsukiichiWatcher/test/TsukiichiWatcher.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "TsukiichiWatcher/src/TsukiichiWatcher"
